...
def create_dataset(dataset_config):
    ...
    elif dataset_config['type'] == 'bicubic_train':
        dataset = BicubicFromSource_train(**dataset_config['params'])
    elif dataset_config['type'] == 'bicubic_val':
        dataset = BicubicFromSource_val(**dataset_config['params'])
    else:
...
class BicubicFromSource_train(DegradedDataFromSource):
    def __getitem__(self, index):
        im_path = self.file_paths[index]
        im_gt = util_image.imread(im_path, chn='rgb', dtype='float32')
        ...
        if not hasattr(self, 'random_cropper'):
            self.random_cropper = util_image.RandomCrop(
                pch_size=self.degrade_kwargs.get('pch_size', 192),
        ...
        im_lq ,im_lq_up= self.bicubic_transform(im_gt)
        im_lq, im_gt,im_lq_up = self.paired_aug([im_lq, im_gt,im_lq_up])

        out = {'gt':self.transform(im_gt), 'lq':self.transform(im_lq),'lq_up':self.transform(im_lq_up)}
        if self.need_path:
            out['path'] = im_path

        return out
class BicubicFromSource_val(DegradedDataFromSource):
    def __getitem__(self, index):
        im_path = self.file_paths[index]
        im_gt = util_image.imread(im_path, chn='rgb', dtype='float32')
