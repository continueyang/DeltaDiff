...
class TrainerBase:
    ...
    def resume_from_ckpt(self):
        ...
        if self.configs.resume:
            ...
            # util_net.reload_model(self.SRmodel, ckpt['SRstate_dict'])
    ...
    def build_model(self):
        ...
        model.cuda()
        # srparams = self.configs.get('network_g', dict)
        # SRmodel = util_common.get_obj_from_str(self.configs.network_gtar.target)(**srparams)
        # SRmodel.cuda()
        if self.configs.model.ckpt_path is not None:
            ...
            if 'state_dict' in ckpt:
                unckpt = ckpt['state_dict']


            util_net.reload_model(model,unckpt)
            # util_net.reload_model(SRmodel, srckpt)
...
class TrainerDifIR(TrainerBase):
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan and phase == 'train':
            ...
            val_maintain = self.configs.train.get('val_maintain', 1)
            for key, value in data.items():
                # print(val_maintain)
                if val_maintain == 1:
                    # 如果 val_maintain 为 1，保持图像原有分辨率，不做任何处理
                    # print(f'vaule{value.shape}')
                    data[key] = value
                else:
                    h, w = value.shape[2:]
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            if self.configs.model.params.cond_lq:
                model_kwargs = {'lq':micro_data['lq'],'lq_up':micro_data['lq_up']}
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            window_size=self.configs.model.get('window_size',8)
            scale=self.configs.network_g.get('upscale',None)
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                num_iters = 0
                if self.configs.model.params.cond_lq:
                    model_kwargs = {'lq':data['lq'],'lq_up':data['lq_up']}
                    if 'mask' in data:
