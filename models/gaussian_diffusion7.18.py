...
from basicsr.useswinir import maininf
...
class STRESDIF:
    ...
    def q_sample(self, x_start, y, t,):
        ...
        return (
            _extract_into_tensor(self.etas_prev, t, x_start.shape) * (y-x_start) + x_start
        )
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        # print( _extract_into_tensor(1-self.etas, t, x_t.shape))

        posterior_mean = (
                              _extract_into_tensor(self.etas, t, x_t.shape) *(  x_start-x_t) +x_t
        )
    ...
    def p_mean_variance(
        ...
        if self.model_mean_type == ModelMeanType.START_X:      # predict x_0
            pred_xstart = model_output #process_xstart(model_output)
    ...
    def p_sample(self, model,SRmodel ,x, y, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        """
        Sample x_{t-1} from the model at the given timestep.

        :param model: the model to sample from.
        :param x: the current tensor at x_t. x是加了15步噪声后 y是低分辨率
        :param y: the [N x C x ...] tensor of degraded inputs.
    ...
    def training_losses(
        ...
        z_start = x_start
        target_size = z_start.shape[2:]

        # 使用双三次上采样（bicubic upsampling）
        z_y = F.interpolate(z_y, size=target_size, mode='bicubic', align_corners=False)
        # if ori_mse:
        #     terms["srmse"] = mean_flat((z_start - z_y) ** 2)


        z_t = self.q_sample(z_start, z_y, t,)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            if ori_mse:
                ...
                terms["l1"]=F.l1_loss(target, model_output, reduction='none')
            # print(terms["mse"])
    ...
    def _scale_input(self, inputs, t):
        if self.normalize_input:
            if self.latent_flag:
                ...
                std = th.sqrt(_extract_into_tensor(self.etas, t, inputs.shape) + 1)
...
class RESDIF:
    ...
    def q_sample(self, x_start, y, t,):
        ...
        # print(f'y.shape:{y.shape}')
        return (
              x_start-_extract_into_tensor(self.etas_prev, t, x_start.shape) * (x_start-y)
        )
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        # posterior_mean = (_extract_into_tensor(self.etas, t, x_t.shape)*(x_start-x_t)+x_t)
        # print(_extract_into_tensor(self.etas, t, x_t.shape))
        assert (
            posterior_mean.shape[0]
            == x_start.shape[0]
        )
...
class Diffusionnoise:
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        assert x_start.shape == x_t.shape
        posterior_mean = (
           _extract_into_tensor(self.posterior_mean_coef1, t, x_t.shape) * x_t +_extract_into_tensor(self.posterior_mean_coef2, t, x_t.shape) *
             x_start
        )#self.posterior_variance = kappa**2 * self.etas_prev / self.etas * self.alpha
    ...
    def p_sample_loop_progressive(
        ...
        z_sample = self.prior_sample(z_y, noise)#加噪 最后版本  =15
    ...
    def training_losses(
            self, model,SRmodel, x_start, y, t,
            first_stage_model=None,
            model_kwargs=None,
            noise=None,
            ):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        terms = {}
        z_y = SRmodel.optimize_parameters(y)
        z_start = x_start
        terms["srmse"] = mean_flat((z_start - z_y) ** 2)
