...
class BaseSampler:
    ...
    def build_model(self):
        ...
        srmodel = util_common.instantiate_from_config(self.configs.network_g).cuda()
...
class RESSampler(BaseSampler):
    def sample_func(self, y0,y0_bic, noise_repeat=False, mask=False):
        '''
        Input:
            y0: n x c x h x w torch tensor, low-quality image, [-1, 1], RGB
            mask: image mask for inpainting
        ...
        if not (ori_h % 8 == 0 and ori_w % 8 == 0):
            ...
            if w % 8 != 0:
                mod_pad_w = 8 - w % 8
            y0 = F.pad(y0, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
            y0_bic = F.pad(y0_bic, (0, mod_pad_w * 4, 0, mod_pad_h * 4), 'reflect')
            flag_pad = True
        else:
            flag_pad = False
        ...
        if self.configs.model.params.cond_lq:
            model_kwargs={'lq':y0,'lq_up':y0_bic}
        else:
            model_kwargs = None

        results = self.base_diffusion.p_sample_loop_progressiveone(
                y=y0,
                model=self.model,
                denoised_fn=None,
                model_kwargs=model_kwargs,
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=True, bs=1, noise_repeat=False):
        ...
        Input:
            ...
            mask_path: image mask for inpainting
        '''
        def _process_per_image(im_lq_tensor,im_lq_up ):
            '''
            Input:
                im_lq_tensor: b x c x h x w, torch tensor, [-1, 1], RGB
                mask: image mask for inpainting, [-1, 1], 1 for unknown area
        ...
        if in_path.is_dir():
            ...
            for data in dataloader:
                ...
                if micro_data['lq'].shape[0] > 0:
                    results = _process_per_image(
                            micro_data['lq'].cuda(),micro_data['lq_up'].cuda(),
    ...
    def build_model(self):
        ...
        self.freeze_model(model)
        self.model = model.eval()
    def load_model(self, model,srmodel ,ckpt_path=None):
        print(ckpt_path)
        state = torch.load(ckpt_path, map_location=f"cuda:{self.rank}")

        if 'state_dict' in state:
            print("st")
            uetstate = state['state_dict']

        # srmodel.load_state_dict(state['state_dict'],strict=True)
        util_net.reload_model(model, uetstate)
